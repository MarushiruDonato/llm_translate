...
- [ ] 初始化 WXT + TypeScript + Preact
- [ ] `wxt.config.ts` 声明：
  - `permissions: ["storage", "contextMenus"]`
  - `optional_host_permissions: ["*://*/*"]`
  - `commands`: `translate-selection`（默认 `Alt+T`）
  - content script：`matches: ["http://*/*", "https://*/*"]`，`all_frames: true`
- [ ] 建立 `src/` 目录骨架（adapters / components / types / utils / storage 空模块）
- [ ] TS `strict` 模式开启；配置 ESLint 基础规则
- [ ] 配置 Vitest（仅纯函数单测）
...
- [ ] **Port 消息契约**（一请求一 Port，请求内串行）：
  - CS → SW：`{ type: "translate", request }`
  - SW → CS：`{ type: "meta", cached, detectedLang }` → `{ type: "chunk", text }*` → `{ type: "done" }` 或 `{ type: "error", message, canRetry }`
  - 保活：`{ type: "ping" }`（CS 忽略）
- [ ] 存储封装（`src/storage/`）：settings / profiles / history（LRU 上限 500）
- [ ] 工具函数（`src/utils/`）：缓存键（SHA-256：文本+模型+目标语言+上下文哈希+Prompt哈希+参数哈希）、URL→MatchPattern 提取
- [ ] 单元测试：LRU 淘汰、缓存键稳定性、MatchPattern 提取（含端口、子路径、非法 URL）
...
- [ ] Options 最简表单：单配置档（name / baseUrl / apiKey / model），存入 storage
- [ ] `openai-messages` 适配器（首发唯一协议）：
  - fetch + `ReadableStream` SSE 解析
  - 非流式端点降级：整段 chunk + done
  - `AbortSignal` 贯穿
- [ ] Background：`onConnect` → 查配置 → 查缓存（命中：meta(cached) + 单 chunk + done）→ 调适配器推流；`onDisconnect` → abort
- [ ] Content Script 最简版：
  - mouseup / selectionchange 检测选区（防抖）
  - 选区旁弹小按钮，点击发起翻译
  - 卡片裸版：纯文本流式渲染
- [ ] 历史写入：done 时落库（供缓存与后续 Popup 复用）
...
- [ ] `openai-responses` 适配器（事件流：`response.output_text.delta` 等）
- [ ] `anthropic` 适配器（`x-api-key` + `anthropic-version` 头，SSE `content_block_delta`，`max_tokens` 必填）
- [ ] 统一错误映射：401/403（key 无效，不可重试）、429（限流，可重试）、5xx/网络（可重试）→ `error` 消息
- [ ] 心跳：非流式等待期间每 20s `ping`
- [ ] Options 增加「测试连接」按钮：固定例句一键验证端点连通（每协议各测一次）
- [ ] SSE 解析器单测：三种协议各准备真实格式的 fixture 数据
...
- [ ] 正式挂载：WXT `createShadowRootUi` + Preact，样式完全封装，`z-index: 2147483647`，内部事件不冒泡
- [ ] 操作完整：复制 / 重试（断开旧 Port 再开新请求，绕缓存）/ 切换模型（所有启用模型扁平下拉，显示「配置档 / 模型」）/ 钉住 / 关闭（外点 / Esc / 按钮）
- [ ] 竞态纪律：**一请求一 Port**；重试或切模型前必须先 disconnect 旧 Port，杜绝双流写入同一卡片
- [ ] 拖动；视口 Flip/Shift 翻转避让（含 iframe 内小视口约束）
- [ ] 深浅色：`prefers-color-scheme`
- [ ] 缓存命中标识（meta.cached）；源语言显示
- [ ] 源语言识别方案（实现期决策）：**`chrome.i18n.detectLanguage` 本地检测**，零成本不占 LLM 流式输出，结果放 `meta.detectedLang`
- [ ] 复制降级：`navigator.clipboard` 失败时回退 `execCommand`
...
- [ ] 右键菜单：`contextMenus.create`（`contexts: ["selection"]`），`onClicked` → `tabs.sendMessage(tab.id, …, { frameId: info.frameId })` 精确路由
- [ ] 快捷键：`commands.onCommand` → 广播该标签页所有 frame（不带 frameId），无选区 frame 静默忽略
- [ ] 设置开关联动：划词按钮 / 右键 / 快捷键三开关，SW 启动时按设置注册菜单与命令处理
- [ ] 全局开关：禁用后三种触发全部静默
- [ ] 设置页：快捷键修改引导按钮 → `chrome://extensions/shortcuts`
...
- [ ] 历史列表（时间倒序）：原文 + 译文 + 模型 + 时间；首屏 20 条，滚动加载
- [ ] 搜索（原文/译文模糊匹配）；单条复制；一键清空（确认弹窗）
- [ ] 全局开关（与 M5 联动）；设置入口
...
- [ ] 配置档管理：增删改、排序；每档内模型列表增删改、启用开关、参数编辑（temperature / max_tokens 等）
- [ ] 默认模型全局唯一标记；卡片模型下拉与配置档数据联动
- [ ] **权限申请流程**：保存配置档时在点击事件处理器内**同步调用** `chrome.permissions.request({ origins: [matchPattern] })`；baseUrl → MatchPattern 自动提取；拒绝授权时明确提示并阻止保存
- [ ] 翻译设置：目标语言（预置常用列表）、系统提示词多行编辑、上下文范围（0–N，默认 0）、流式开关
- [ ] 触发设置：三开关 + 快捷键引导（M5 已建，此处收口）
- [ ] 历史设置：清空按钮
...
- [ ] 图标三规格：`icons/icon-16.png` / `icon-48.png` / `icon-128.png`（独立绘制，非单图缩放）
- [ ] `CHROMEWEBSTORE.md`：名称、一句话与详细描述、权限理由（storage / contextMenus / optional_host_permissions）、隐私声明（零后端、零收集、BYOK 直连）
- [ ] 全量冒烟清单（`test/SMOKE.md`）：普通页 / iframe 重站点 / 深色站点 / 慢端点流式 / 断网重试 / 三触发方式 / 缓存命中 / 历史上限淘汰 / 卸载重装
- [ ] `wxt zip` 产出商店包
